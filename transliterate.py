...
# def modified_levenshtein(s, t):
    ...
    # res = min([modified_levenshtein(s[:-1], t)+1,
               # modified_levenshtein(s, t[:-1])+1, 
               # modified_levenshtein(s[:-1], t[:-1]) + cost])
    # print("res", res)
    # sys.exit("done")
    # return res

def policy(s, t, costs=(1, 1, 1)):
    """ 
...
# def policy(s, t):
    ...
    # res = min([(2*modified_levenshtein(s[:-1], t))+1,
               # (2*modified_levenshtein(s, t[:-1]))+1, 
               # (0.5*modified_levenshtein(s[:-1], t[:-1])) + cost])
    # print("res", res)
    # sys.exit("done")
    # return res

# Train the model on one example
def train(input_variable, target_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, reward, max_length=20):
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            ...
            loss += criterion(decoder_output, target_variable[di])+reward_loss
...
def generateRandomlytrain(encoder, decoder, n=5):
    score = 0
    # pair2 = []
    for i in range(n) :
        ...
        score += policy(''.join(output_chars).replace('<EOS>', ''),pair[1])

    return score

# Full training process_reinforce
def trainItersreinforce(pairs, input_lang, output_lang, encoder, decoder, n_iters, print_every, plot_every, learning_rate):
    start = time.time()
    plot_losses = []
...
def trainIters(pairs, input_lang, output_lang, encoder, decoder, n_iters, print_every=100, plot_every=1000, learning_rate=0.01):
    ...
    for iter in range(1, n_iters + 1):
        ...
        # train on one example
        loss = train(input_variable, target_variable, encoder,
                     decoder, encoder_optimizer, decoder_optimizer, criterion, reward)
        # print("loss", loss)
...
def generateTest(encoder, decoder, test_pairs):
    ...
    for pair in test_pairs:
        output_chars, attentions = generate(encoder, decoder, pair[0])
        # score += modified_levenshtein(''.join(output_chars).replace('<EOS>', ''),pair[1])
        score += edit_distance(''.join(output_chars).replace('<EOS>', ''),pair[1])
        # score_reinforcement = reinforcement(score)
        # score += edit_distance.eval(''.join(output_chars).replace('<EOS>', ''),pair[1])
        # print("source length", len(pair[1]))
        # print("target length", len(output_chars))
...
if __name__ == "__main__":
    ...
    decoder = AttnDecoderRNN(hidden_size, output_lang.n_chars, 1, dropout_p=0.2)
    if use_cuda:
